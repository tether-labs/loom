...
pub fn init(target: *ThreadRipper, options: Options) !void {
    ...
    const action_list = try options.arena.create(ActionList);
...
fn register_ctx(noalias tr: *ThreadRipper, noalias thread: *Thread_Ctx) void {
    // Push the thread onto the threads stack in a lock-free manner.
    var threads_stack_ctx = tr.threads_stack_ctx.load(.monotonic);
    // Threads is a stack we add the thread to the front of the stack
    while (true) {
        thread.next = threads_stack_ctx;
        threads_stack_ctx = tr.threads_stack_ctx.cmpxchgStrong(
            threads_stack_ctx,
            thread,
            .release,
            .monotonic,
...
fn unregister_ctx(noalias tr: *ThreadRipper, noalias maybe_thread: ?*Thread_Ctx) void {
...
pub fn addToBatch(tr: *ThreadRipper, node: *Node) !void {
    if (tr.batch_size > 0) {
        const current_head = tr.batch.head;
        node.next = current_head;
        tr.batch.head = node;
        tr.batch_size += 1;

        if (tr.batch_size > 50) {
            tr.dispatchBatch(tr.batch);
            tr.batch_size = 0;
        }
        return;
    }
...
const Thread_Ctx = struct {
    id: usize,
    next: ?*Thread_Ctx = null,
    target: ?*Thread_Ctx = null,
    join_event: Event = .{},
    action_queue: ActionQueue = .{},
    injector: Injector = .{},

    threadlocal var current: ?*Thread_Ctx = null;

    fn worker(tr: *ThreadRipper, id: usize) !void {
        var thread = Thread_Ctx{ .id = id };
        current = &thread;
        print("{any}\n", .{id});

        // add the thread to the threads linkedlist
        tr.register_ctx(&thread);
        defer tr.unregister_ctx(&thread);
        ...
        while (true) {
            ...
            while (thread.pop(tr)) |stolen_node| {
                ...
                const runFnCtx = run_node.data.runFn;
                is_waking = false;
                try runFnCtx(&run_node.data, tr.ctxs[thread.id]);
            } else {}
        }
    }
    ...
    fn pop(noalias thread: *Thread_Ctx, noalias tr: *ThreadRipper) ?ActionQueue.Thief {
        ...
        while (num_threads > 0) : (num_threads -= 1) {
            ...
            const target = thread.target orelse tr.threads_stack_ctx.load(.acquire) orelse unreachable;
